...
@section('page-title', 'Record Task')
    ...
    <div class="col-lg-8 mx-auto">
        <div class="card">
            <div class="card-header">
                <h5 class="mb-0"><i class="fas fa-list-check me-2"></i>New Task</h5>
            </div>
            <div class="card-body">
                        ...
                        <div class="card-body">
                            <div class="row">
                                <div class="col-md-6 mb-3" id="title-wrapper">
                                    ...
                                    <input type="text" class="form-control @error('title') is-invalid @enderror" id="title" name="title" value="{{ old('title') }}" placeholder="Generating auto-generated title..." style="background-color: #f8f9fa;">
                                ...
                                <div class="col-md-3 mb-3">
                                    ...
                                    <input type="date" class="form-control @error('work_date') is-invalid @enderror" id="work_date" name="work_date" value="{{ old('work_date', today()->toDateString()) }}" required>
                                    @error('work_date')
                                        <div class="invalid-feedback">{{ $message }}</div>
                                    @enderror
                                </div>
                            ...
                            <div class="row">
                                <div class="col-md-3 mb-3" id="quantity-wrapper">
                                    ...
                                    <input type="number" step="0.01" min="0" class="form-control @error('quantity_kg') is-invalid @enderror" id="quantity_kg" name="quantity_kg" value="{{ old('quantity_kg') }}" placeholder="e.g., 25.5">
                                ...
                                <div class="col-md-3 mb-3 d-none" id="tree-count-wrapper">
                                    ...
                                    <input type="number" step="1" min="0" class="form-control @error('tree_count') is-invalid @enderror" id="tree_count" name="tree_count" value="{{ old('tree_count') }}" placeholder="e.g., 150">
                                ...
                                <div class="col-md-3 mb-3">
                                    ...
                                    <select class="form-select @error('status') is-invalid @enderror" id="status" name="status" required>
                                        @foreach($statuses as $key => $label)
                                            <option value="{{ $key }}" {{ old('status', 'planned') == $key ? 'selected' : '' }}>{{ $label }}</option>
                                        @endforeach
                                    </select>
                                    @error('status')
                                        <div class="invalid-feedback">{{ $message }}</div>
                                ...
                                <div class="col-md-3 mb-3">
                                    <div class="form-check mt-4">
                                        <input type="checkbox" class="form-check-input" id="mark_complete" name="mark_complete" value="1" {{ old('mark_complete') ? 'checked' : '' }}>
                                        <label class="form-check-label" for="mark_complete">
                                            Mark as Complete
                                            <i class="fas fa-question-circle ms-1" style="cursor: help; font-size: 0.85rem; opacity: 0.6;" data-bs-toggle="tooltip" title="For harvesting tasks, check this to mark complete and sync to harvest batch immediately"></i>
                                        </label>
                                    </div>
                                    <div class="form-text d-none" id="harvest-sync-info">
                                        <i class="fas fa-check text-success me-1"></i>Will sync to harvest batch
                                    </div>
                                </div>
                            </div>
                        </div>
                        ...
                        <div class="card-body">
                            <div class="mb-3">
                                ...
                                <select class="form-select d-none @error('workers') is-invalid @enderror" id="workers" name="workers[]" multiple>
                                    @foreach($workers as $worker)
                                        <option value="{{ $worker->id }}" {{ collect(old('workers', []))->contains($worker->id) ? 'selected' : '' }}>{{ $worker->name }} ({{ ucfirst(str_replace('_', ' ', $worker->role)) }})</option>
                                    @endforeach
                                </select>
                                @error('workers')
                                    <div class="invalid-feedback">{{ $message }}</div>
                        ...
                        <div class="card-body">
                            <div class="row">
                                <div class="col-md-6 mb-3">
                                    ...
                                    <input type="number" step="0.01" min="0" class="form-control @error('rate') is-invalid @enderror" id="rate" name="rate" value="{{ old('rate') }}" placeholder="Rate per kg or per job">
                                ...
                                <div class="col-md-6 mb-3">
                                    ...
                                    <input type="text" class="form-control" id="cost_display" value="RM0.00" placeholder="RM0.00">
                                    <input type="hidden" name="cost" id="cost_value" value="0">
                    ...
                    <div class="card mb-4">
                        ...
                        <div class="card-body">
                            <div class="mb-3">
                                ...
                                <textarea class="form-control @error('description') is-invalid @enderror" id="description" name="description" rows="4" placeholder="Describe the work done...">{{ old('description') }}</textarea>
                                @error('description')
                                    <div class="invalid-feedback">{{ $message }}</div>
                                @enderror
                            </div>
...
(function() {
    ...
    function initializeForm() {
        ...
        const markCompleteCheckbox = document.getElementById('mark_complete');
        const harvestSyncInfo = document.getElementById('harvest-sync-info');
        ...
        function updateVisibilityAndDefaults() {
            ...
            // Quantity only relevant for harvesting
            if (qtyWrap) qtyWrap.style.display = type === 'harvesting' ? 'block' : 'none';

            // Tree count only relevant for manuring
            if (treeWrap) {
            ...
            // Show/hide mark complete checkbox only for harvesting
            if (markCompleteCheckbox) markCompleteCheckbox.parentElement.parentElement.style.display = type === 'harvesting' ? 'block' : 'none';

            // For harvesting: auto-check and disable the mark complete checkbox
            if (markCompleteCheckbox) {
                if (type === 'harvesting') {
                    markCompleteCheckbox.checked = true;
                    markCompleteCheckbox.disabled = true;
                    markCompleteCheckbox.style.cursor = 'not-allowed';
                    if (statusEl) statusEl.value = 'completed';
                    if (harvestSyncInfo) harvestSyncInfo.classList.remove('d-none');
                } else {
                    markCompleteCheckbox.checked = false;
                    markCompleteCheckbox.disabled = false;
                    markCompleteCheckbox.style.cursor = 'pointer';
                    if (harvestSyncInfo) harvestSyncInfo.classList.add('d-none');
                }
            }

            // Default rates for common jobs
