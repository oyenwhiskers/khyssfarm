                    ...
                    <div class="row">
                        ...
                        <div class="col-md-6 mb-3">
                            ...
                            <select class="form-select @error('marketing_campaign_id') is-invalid @enderror" id="marketing_campaign_id" name="marketing_campaign_id">
                                ...
                                @php
                                    $activeCampaigns = \App\Models\Marketing::where('status', 'active')->orWhere('id', $customer->marketing_campaign_id)->orderBy('campaign_name')->get();
                                @endphp
                                @foreach($activeCampaigns as $campaign)
                                    <option value="{{ $campaign->id }}" {{ (old('marketing_campaign_id') ?? $customer->marketing_campaign_id) == $campaign->id ? 'selected' : '' }}>
                                        {{ $campaign->campaign_name }} {{ $campaign->status != 'active' ? '(Ended)' : '' }}
                                    </option>
                                @endforeach
                            </select>
